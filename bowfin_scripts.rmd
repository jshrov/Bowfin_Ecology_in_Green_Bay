...
BON <- read.csv("bowfinbc.csv")

# Von Bert
vblog <- function(T, logLinf, K = NULL, t0 = NULL) {
  # If passed as list
  if(length(logLinf) == 3) {
    t0 <- logLinf[[3]]
    K <- logLinf[[2]]
...
# Richards
richlog <-  function(T, logLinf, k = NULL, tstar = NULL, b = NULL) {
  # If passed as list
  if(length(Linf) == 4) {
    b <- Linf[[4]]
    tstar <- Linf[[3]]
...
fitL.01 <- nlme(BC.Len ~ lgstclog(BC.Age, logLinf, tstar, gninf), data = BON,
...
# fitL.01p <- nlme(BC.Len ~ lgstclog(BC.Age, logLinf, tstar,gninf), data = BON,
#                 fixed = list(logLinf ~ 1, tstar ~ 1, gninf ~ 1),
#                 random = logLinf + tstar + gninf~1 | ID,
#                 start = list(fixed = c(logLinf = log(600), tstar = 2, gninf = 0.8)),
#                 weights = varPower(form = ~ BC.Age))
